functions {
  ...
  matrix add_diag_jitter(matrix A, real jitter) {
    int m = rows(A);
    matrix[m, m] out = A;
    for (i in 1 : m) {
      out[i, i] += jitter;
  ...
  matrix sqrtm(matrix A) {
    int m = cols(A);
    matrix[m, m] A_sym = symmetrize(A);
    vector[m] eigenvals = eigenvalues_sym(A_sym);
    vector[m] sq_eigenvals;
    matrix[m, m] eigenvecs = eigenvectors_sym(A_sym);
    matrix[m, m] product_1;
    matrix[m, m] out;
    for (i in 1 : m) {
      sq_eigenvals[i] = sqrt(fmax(eigenvals[i], 1e-12));
    }
    product_1 = diag_post_multiply(eigenvecs, sq_eigenvals);
    out = mdivide_right(product_1, eigenvecs);
    return out;
  }
  ...
  matrix A_to_P(matrix A) {
    ...
    matrix[m, m] B;
    matrix[m, m] P;
    C = add_diag_jitter(symmetrize(diag_matrix(rep_vector(1, m)) + tcrossprod(A)), 1e-12);
    B = sqrtm(C);
  ...
  array[,] matrix P_to_AR(array[] matrix P, matrix Sigma) {
    ...
    for (s in 0 : (p - 1)) {
      Y = add_diag_jitter(symmetrize(diag_matrix(rep_vector(1, m)) - tcrossprod(P[p - s])), 1e-10);
      sqY = sqrtm(Y);
...
data {
  ...
  // Add the new hyperparameters for the gamma distribution rate parameter for lambda
  real<lower=0> c1;  // Add c1 here
  real<lower=0> c2;  // Add c2 here
}
transformed data {
  vector[m] mu;
...
parameters {
  ...
  array[p] matrix<lower=0>[m, m] lambda; // The lambda parameters
  array[p] matrix<lower=0>[m, m] r;      // The rate parameters for lambda
}
transformed parameters {
  ...
  G = add_diag_jitter(symmetrize(G), 1e-8);
  {
    vector[p] logtau;
...
model {
  ...
  for (s in 1 : p) {
    for (i in 1 : m) {
      for (j in 1 : m) {
        // Lambda follows Gamma(a/2, r[s, i, j])
        lambda[s, i, j] ~ gamma(a / 2, r[s, i, j]);

        // A[s, i, j] follows normal distribution with standard deviation determined by lambda
        A[s, i, j] ~ normal(0, 1 / sqrt(lambda[s, i, j] * tau[s]));

        // The rate parameter r follows Gamma(c1, c2)
        r[s, i, j] ~ gamma(c1, c2);
      }
    }
  }
